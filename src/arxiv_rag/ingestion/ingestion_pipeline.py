...
MAX_ADD_ATTEMPTS = 5
ADD_RETRY_DELAY_SECONDS = 30


def _corpus_hash(arxiv_ids: list[str]) -> str:
    return hashlib.sha256("".join(arxiv_ids).encode("utf-8")).hexdigest()
...
def _add_documents_with_retry(vector_store: VectorStore, documents: list[Document]) -> None:
    for attempt in range(1, MAX_ADD_ATTEMPTS + 1):
        try:
            ...
            log.warning("embedding attempt %d/%d failed; retrying in %d seconds: %s",
                        attempt, MAX_ADD_ATTEMPTS, ADD_RETRY_DELAY_SECONDS, error)
            time.sleep(ADD_RETRY_DELAY_SECONDS)
