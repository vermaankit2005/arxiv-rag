...
class ChromaStore(vector_store_contract.VectorStore):
    def __init__(self, embeddings: Embeddings, persist_directory: Path = CHROMA_DIRECTORY,
    ...
    @classmethod
    def open(cls, embeddings: Embeddings, create_if_missing: bool = False, staging: bool = False, collection_name: str | None = None) -> "ChromaStore":
        ...
        if not create_if_missing and not CHROMA_DATABASE_FILE.exists():
            raise FileNotFoundError(f"No Chroma database found at {CHROMA_DIRECTORY}. Run ingestion first.")

        if collection_name is None:
            collection_name = vector_store_contract.versioned_collection_name(CHROMA_COLLECTION_NAME) if staging else cls._get_active_collection_name()
        log.info("opening Chroma collection %s", collection_name)
        return ChromaStore(embeddings=embeddings, collection_name=collection_name)
...
def get_chroma_store(embeddings: Embeddings, create_if_missing: bool = False, staging: bool = False, collection_name: str | None = None) -> vector_store_contract.VectorStore:
    return ChromaStore.open(
