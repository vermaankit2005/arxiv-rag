...
def _build_prompt(question: str, context: RetrievalContext, answer_mode: AnswerMode = "standard") -> str:
    ...
    return (
        ...
        f"{mode_rules}\n"
        "Rules:\n"
        "- Answer directly in natural prose. Use Markdown only when it genuinely improves readability.\n"
        "- Lead with the answer; do not announce what will follow or repeat the question as an introduction.\n"
        "- For a short factual answer, use one compact sentence or paragraph.\n"
        "- Do not add a heading when the answer fits in one or two paragraphs.\n"
        "- Use bullets only for several independent items or steps that are genuinely easier to scan.\n"
        "- Match the amount of detail and structure to the question, and stop once the requested points are answered.\n"
