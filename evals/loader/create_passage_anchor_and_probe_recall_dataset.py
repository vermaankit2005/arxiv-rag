...
LOADED_PROBES_DATASET_PATH = (
    Path(__file__).parents[1] / "data" / "recall_probes_dataset.json"
)


def create_passage_anchor_and_probe_recall_dataset():
    papers = json.loads(PAPERS_JSON_PATH.read_text(encoding="utf-8"))
    loaded_probes_dataset = json.loads(
    ...
    for paper in papers:
        ...
        matching_dataset = next(
            (
                dataset
                for dataset in loaded_probes_dataset
                if dataset["arxiv_id"] == paper["arxiv_id"]
            ),
            None,
        )
