...
def anchor_coverage():
    ...
    for paper in json_data:
        html_content = (HTML_PATH / f"{paper['arxiv_id']}.html").read_text(encoding="utf-8")
        loaded = load(paper["arxiv_id"], None)  # Assuming load can handle None for client
        ...
        for passage in loaded.passages:
            ...
            if passage.location:
                ...
                if f'id="{anchor}"' in html_content:
                    ...
                    print(f"Anchor not found in HTML: {passage.location} for passage: {passage.text[:60]}...")

    print(f"Total anchors found in HTML: {loaded_anchor_found_in_html}")
    print(f"Metrics Anchor Coverage: {loaded_anchor_found_in_html / loaded_passage_count * 100:.2f}%")
