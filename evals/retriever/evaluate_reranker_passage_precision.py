"""This checks how many of the final 15 reranked passages contain required evidence. The formula is: relevant passages / 15."""
...
EXPERIMENT_PREFIX = "reranker-passage-precision-at-15"
EXPERIMENT_METADATA = {
    "metric": "passage_precision_at_15",
...
def evaluate_passage_precision(outputs: dict, reference_outputs: dict) -> dict:
    relevant_passages = sum(
        _passage_matches_evidence(passage, reference_outputs)
        for passage in outputs.get("passages", [])
    )
    score = relevant_passages / RERANK_TOP_N

    return {
        "key": "passage_precision_at_15",
        "score": score,
        "comment": f"Found required evidence in {relevant_passages}/{RERANK_TOP_N} reranked passages.",
    }


def run_passage_precision() -> None:
    load_dotenv()
    client = Client()
    ...
    results = client.evaluate(
        ...
        evaluators=[evaluate_passage_precision],
    ...
    if not upload_results:
        ...
        print_local_score(completed_results, "passage_precision_at_15")


if __name__ == "__main__":
    run_passage_precision()
